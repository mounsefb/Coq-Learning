...
Proof.
  (* FILL IN HERE Admitted. *)
  induction n, m; simpl; try reflexivity.
...
Theorem plus_Sn_Sm : forall n m : nat,
  S n + S m = S(S(n + m)).
Proof.
  (* FILL IN HERE Admitted. *)
  induction n, m.
...
Proof.
  ...
  induction n, m, p.
    ...
    - simpl. repeat rewrite <- plus_n_Sm. rewrite IHn. rewrite plus_Sn_m. reflexivity.
Qed.
(** [] *)
...
(** **** Exercise: 2 stars, advanced, especially useful (add_comm_informal)
  ...
  _Theorem_: For any [n], [m]
    ...
    - First, suppose [n = 0].  We must show that
      ...
      This follows directly from the definition of [+] where 0 is the neutral element.
    ...
    - Next, suppose [n = S n'], where
      ...
      Using the equivalent of add_Sn_m, we get :

        (S n' + m ) = S (n' + m) = S (m + n') by induction hypothesis 
                    = (m + S n') with add_n_Sm
*)
...
Theorem mult_m_Sn : forall m n: nat, m * S n = m + m*n.
Proof.
  intros m n.
  induction m.
    - simpl. reflexivity.
...
Proof.
  ...
  induction b.
    - simpl. reflexivity.
    - simpl. rewrite add_0_r. rewrite plus_n_Sm. rewrite bin_to_nat_S. 
      assert( H: bin_to_nat b + 1 = 1 + bin_to_nat b).
        { rewrite add_comm. reflexivity. }
      rewrite <- H.
      rewrite add_assoc'. reflexivity.
    -simpl. rewrite add_0_r. rewrite plus_n_Sm. rewrite IHb. rewrite add_assoc'. 
    rewrite add_0_r. 
    assert( H: bin_to_nat b + 1 = 1 + bin_to_nat b).
      { rewrite add_comm. reflexivity. }
    assert( H1: 1 + bin_to_nat b + 1 = 2 + bin_to_nat b).
      { rewrite add_comm. reflexivity. }
    assert( H2: bin_to_nat b + bin_to_nat b + 2 = 2 + bin_to_nat b + bin_to_nat b).
      { rewrite add_comm. reflexivity. }
