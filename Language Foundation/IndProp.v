...
Proof.
  ...
  rewrite <- add_assoc in H1.
  rewrite (add_comm m (n+p)) in H1.                          
  rewrite add_assoc in H1.
  rewrite (add_assoc n n p) in H1.
...
Proof.
 ...
 induction n1.
   ...
   ** apply le_trans with (n := S n1 + n2).
    *** apply le_plus_l.
    *** apply H.
   ** rewrite plus_Sn_m in H. apply le_S in H. apply Sn_le_Sm__n_le_m in H. apply IHn1 in H.
    destruct H. apply H0.
Qed.   
...
(* FILL IN HERE Admitted. *)
  ...
  induction n.
    - left. apply O_le_n.
    - destruct p.
      ** simpl in H. simpl in IHn. apply le_S in H. apply Sn_le_Sm__n_le_m in H.
         specialize (IHn H).
         destruct IHn.
         assert ( Hz : n <= 0 -> n = 0 ).
         {
         ...
         apply Hz in H0.
         rewrite H0 in H.
         rewrite plus_O_n in H; right. apply H. right. apply H0.
      ** rewrite plus_Sn_m in H. 
        assert( Heq : S (n + m) <= S p + q -> n + m <= S p + q ).
        {
          intro.
        ...
        apply IHn in H.
        destruct H.
         *** left. apply n_le_m__Sn_le_Sm. Abort. 
...
Theorem le_n_S : forall a b : nat, a <= b -> S a <= S b.
Proof.
  intros.
  induction H.
    * reflexivity.
...
(* FILL IN HERE Admitted. *)
  ...
  destruct m.
    ...
    * unfold lt in H.
    apply Sn_le_Sm__n_le_m in H.
    apply plus_le in H.
    destruct H. apply le_n_S in H. apply le_n_S in H0.
    split.
      ** unfold lt. apply H.
      ** unfold lt. apply H0.
Qed.
...
Proof.
  ...
  - intros.
    ...
    * simpl. rewrite IHexp_match.
      assert ( Horr : forall x, x || true = true ).
      {
        intro.
        destruct x; simpl; reflexivity.
      }
        rewrite Horr. reflexivity.
    * simpl. reflexivity.
    * simpl. reflexivity.
  - intro.
    induction re.
...
Proof.
   ...
   * split.
    ** intro. reflexivity.
    ** intro. apply H.
   * unfold not in H. split.
    ** intro. apply H in H0. exfalso. apply H0.
    ** intro. discriminate.
Qed.
(** [] *)
