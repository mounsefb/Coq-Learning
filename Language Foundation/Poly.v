...
Proof.
  ...
  induction l.
    - simpl. reflexivity.
    - assert( H1 : x::l = x::nil ++ l ).
    {
      simpl.
      reflexivity.
    }
    rewrite H1. simpl. rewrite IHl. reflexivity.
Qed.
...
Fixpoint split {X Y : Type} (l : list (X*Y)) : (list X) * (list Y) :=
  ...
  match l with
  | nil => (nil, nil)
  | (x1, x2)::n => (x1::(fst (split n)), x2::(snd (split n)))
  end.
...
Definition filter_even_gt7 (l : list nat) : list nat :=
  (* REPLACE THIS LINE WITH ":= _your_definition_ .". Admitted. *)
  filter even ( filter (fun x => (leb 7 x)) l ).
...
Proof.
  intros.
  simpl.
  unfold filter_even_gt7. unfold filter. simpl. reflexivity.
Qed.
...
Theorem map_dev : forall (X Y : Type) (f : X -> Y) (l1 l2: list X),
...
Proof.
  ...
  induction l.
    - simpl. reflexivity.
    - simpl. rewrite <- IHl. rewrite map_dev. simpl. reflexivity.
Qed.
(** [] *)
