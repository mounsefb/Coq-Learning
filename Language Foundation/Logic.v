...
Proof.
  intro n.
  destruct n.
    - simpl. intros m H. split; try reflexivity. apply H.
...
Proof.
  ...
  intros P Q.
  repeat unfold not.
  intro H. intro H'. intros. apply H in H0. apply H' in H0. apply H0.
Qed.
...
Proof.
  ...
  unfold not.
  split.
    - unfold not in H. intro. apply H. left. apply H0.
...
Definition disc_fn_nil X (xs: list X) : Prop :=
  match xs with
  | nil => True
  | h::t => False
  end.
...
Proof.
  (* FILL IN HERE Admitted. *)
  intros. intro H.
  assert (H1 : disc_fn_nil X nil). { simpl. apply I. }
...
Proof.
  ...
  intro. destruct H. apply H0 in H1. apply H2. apply H1.
Qed. 
...
Proof.
  ...
  split.
    - intro. destruct H.
      ** split; left; apply H. 
      ** split; destruct H; right; try apply H0. apply H.
    - intro. destruct H. destruct H. destruct H0.
      ** left; apply H.
      ** left; apply H.
      ** destruct H0.
        *** left. apply H0.
        *** right. split; try apply H. apply H0.
Qed. 
...
Proof.
  ...
  - induction l as [|x l' IHl'].
    (* FILL IN HERE Admitted. *)
    ** simpl. intro contra. contradiction.
  ...
  - intros. repeat destruct H. apply In_map. apply H0.
Qed. 
...
Proof.
  ...
  (* FILL IN HERE Admitted. *)
    - simpl. split. 
      ** intro. right. apply H.
      ** intro. destruct H. contradiction. apply H.
    - simpl. split.
      ** intros. destruct H. left. left. apply H.
      apply IH in H. rewrite <- or_assoc. right. apply H.
      ** intro. rewrite IH. rewrite <- or_assoc in H. apply H.
Qed.
(** [] *)
...
Proof.
  ...
  intros. induction l.
    - split.
      ** simpl. intro H. apply I.
      ** simpl. intro. contradiction.
    - simpl. split.
      **  intro H. split.
       *** apply H. left. reflexivity.
       *** apply IHl. intro x0. intro H0. apply H. right. apply H0.
      ** intro H. destruct H. intros. destruct H1.
...
Lemma even_double_conv : forall n, exists k,
  ...
  induction n.
    ...
    - destruct IHn. destruct (even n) eqn:En.
      ** rewrite even_S. rewrite En. simpl. exists x. rewrite H. reflexivity.
      ** rewrite even_S. rewrite En. simpl. exists (S x). simpl. rewrite H. reflexivity.
Qed.  
...
Proof.
  ...
  split.
    ...
    - intro H. destruct H. rewrite H. rewrite H0. simpl. reflexivity. 
Qed.
...
Theorem eqb_neq : forall x y : nat,
  ...
  Proof.
    ...
    split.
    -  unfold not. intros.
      rewrite H0 in H. rewrite eqb_refl in H. 
      discriminate.
    - intros H_neq.
    destruct (x =? y) eqn:H.
    + apply eqb_eq in H. apply H_neq in H. contradiction.
    + reflexivity.
Qed.
...
(* FILL IN HERE Admitted. *)
  induction l1.
    - intros. split.
      ** unfold eqb_list. destruct l2.
        ***intro. reflexivity.
        *** intro. discriminate.
      ** intro. unfold eqb_list. destruct l2.
        *** reflexivity.
        *** discriminate.
    - intros. split. 
      ** unfold eqb_list. destruct l2.
         *** intro. discriminate.
         *** rewrite andb_true_iff. rewrite H. 
          intro. destruct H0. rewrite H0. apply IHl1 in H1. rewrite H1.
          reflexivity.
      ** destruct l2.
        *** discriminate.
        *** intro. rewrite <- H0. simpl. rewrite andb_true_iff. split.
          **** apply H. reflexivity.
...
Theorem app_sym : forall {X:Type} (l1 l2 : list X),
  l1++l2 = l2++l1.
Proof.
  induction l1.
    - simpl. intro. rewrite app_nil_r. reflexivity.
    - simpl. intro l2. simpl. Abort.
...
Proof.
  ...
  induction l as [| x l' IHl].
  - simpl. reflexivity.
  - simpl. rewrite <- IHl. Search rev. Abort.
(* Qed. *)
...
Proof. 
  ...
  - right. rewrite H. unfold not. intro Hft. discriminate.
Qed. 
